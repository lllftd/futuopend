...
# Preflight (Step 1 / scheme D): PYTHONPATH=. python3 scripts/verify_l1a_artifacts.py
#
...
if [[ ! -f "${MODEL_DIR}/l1a_outputs.pkl" ]]; then
  ...
  echo "  Train L1a first, e.g.: ./scripts/run_train.sh layer1a"
  exit 1
fi
...
echo ">> verify_l1a_artifacts.py"
"$PY" -u "${ROOT}/scripts/verify_l1a_artifacts.py" || exit 1
