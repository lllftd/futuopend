...
def _compute_block_labels(
    *,
    ...
    # Compress = quiet vol contraction (low RV z, negative slope, width fading, not volume-spiky).
    # Mean-revert = fade from elevated z (z_lkb high) with moderate z; allow more participation than compress.
    volz_max_compress = float(os.environ.get("L1A_VOL_REG_COMPRESS_VOLZ_MAX", "0.35"))
    volz_min_mr = float(os.environ.get("L1A_VOL_REG_MR_VOLZ_MIN", "-1.0"))
    # Near threshold, relax volume gate so tiny vz noise does not flip compress↔MR (default ±0.05).
    volz_buf = float(os.environ.get("L1A_VOL_REG_VOLZ_BUFFER", "0.05"))
    vz = np.zeros(len(rv), dtype=np.float64) if vol_z is None else np.asarray(vol_z, dtype=np.float64)
    ...
    compress_core = (rv_z < compress_z) & (rv_slope < 0.0) & (bb_chg < bb_drop)
    vol_ok_compress = (vz < volz_max_compress) | (np.abs(vz - volz_max_compress) <= volz_buf)
    compress = compress_core & vol_ok_compress
    mr_core = (
        (np.abs(rv_z) < mr_z) & (rv_slope < 0.0) & (z_lkb > mr_z) & np.isfinite(z_lkb)
    )
    vol_ok_mr = (vz > volz_min_mr) | (np.abs(vz - volz_min_mr) <= volz_buf)
    mean_revert = mr_core & vol_ok_mr
...
def compute_vol_regime_labels(df: pd.DataFrame) -> pd.Series:
    ...
    if "symbol" in df.columns:
        for _, g in df.groupby("symbol", sort=False):
            ...
            vz_g = None
            if "pa_volume_zscore" in g.columns:
                vz_g = pd.to_numeric(g["pa_volume_zscore"], errors="coerce").fillna(0.0).to_numpy(dtype=np.float64)
            lab = _compute_block_labels(
                rv=rv_g, high=h_g, low=l_g, close=c_g, lookback=lookback, rv_win=rv_win, vol_z=vz_g
            )
            out.loc[idx] = lab
    else:
        ...
        vz = None
        if "pa_volume_zscore" in df.columns:
            vz = pd.to_numeric(df["pa_volume_zscore"], errors="coerce").fillna(0.0).to_numpy(dtype=np.float64)
        out.iloc[:] = _compute_block_labels(rv=rv, high=h, low=l, close=c, lookback=lookback, rv_win=rv_win, vol_z=vz)
    return out
