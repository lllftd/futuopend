from __future__ import annotations

import os
import pickle
from dataclasses import dataclass
from typing import Any

import numpy as np
import pandas as pd

from core.training.common.constants import (
    CAL_END,
    L1B_EXPAND_OOF_VAL_WINDOWS,
    L1_EXPAND_OOF_VAL_WINDOWS,
    MODEL_DIR,
    TEST_END,
    TRAIN_END,
)


@dataclass(frozen=True)
class StackTimeSplits:
    train_mask: np.ndarray
    cal_mask: np.ndarray
    test_mask: np.ndarray
    l2_train_mask: np.ndarray
    l2_val_mask: np.ndarray


def l2_val_start_time() -> np.datetime64:
    raw = os.environ.get("L2_VAL_START", "2024-01-01").strip() or "2024-01-01"
    return np.datetime64(raw)


def l1_oof_mode_from_env() -> str:
    """``expanding`` (default): calendar expanding-window OOF per ``L1_EXPAND_OOF_VAL_WINDOWS``.

    ``blocked``: legacy contiguous time blocks inside train+cal (row count K-way split).
    """
    raw = (os.environ.get("L1_OOF_MODE", "expanding") or "expanding").strip().lower()
    return raw if raw in {"expanding", "blocked"} else "expanding"


def l1_expand_oof_val_windows() -> list[tuple[np.datetime64, np.datetime64]]:
    pairs: list[tuple[np.datetime64, np.datetime64]] = []
    for a, b in L1_EXPAND_OOF_VAL_WINDOWS:
        pairs.append((np.datetime64(str(a).strip()), np.datetime64(str(b).strip())))
    if not pairs:
        raise RuntimeError("L1_EXPAND_OOF_VAL_WINDOWS is empty.")
    for vs, ve in pairs:
        if not (vs < ve):
            raise ValueError(f"Invalid expanding OOF window {vs} .. {ve}")
    return pairs


def l1a_expand_oof_val_windows_from_env() -> list[tuple[np.datetime64, np.datetime64]] | None:
    """Optional L1a-only expanding OOF windows; does not change L1b/L2 stack constants.

    Env ``L1A_EXPAND_OOF_VAL_WINDOWS``: comma-separated ``start:end`` pairs (``[start,end)`` val segments),
    e.g. ``2022-07-01:2024-01-01,2024-01-01:2024-07-01`` for two folds. Empty = use ``L1_EXPAND_OOF_VAL_WINDOWS``.
    """
    raw = (os.environ.get("L1A_EXPAND_OOF_VAL_WINDOWS", "") or "").strip()
    if not raw:
        return None
    out: list[tuple[np.datetime64, np.datetime64]] = []
    for segment in raw.split(","):
        segment = segment.strip()
        if not segment:
            continue
        if ":" not in segment:
            raise ValueError(
                "L1A_EXPAND_OOF_VAL_WINDOWS segments must be start:end, e.g. 2022-01-01:2023-01-01. "
                f"Got: {segment!r}"
            )
        a, b = segment.split(":", 1)
        vs, ve = np.datetime64(a.strip()), np.datetime64(b.strip())
        if not (vs < ve):
            raise ValueError(f"Invalid L1a OOF window {vs} .. {ve} (need start < end)")
        out.append((vs, ve))
    if not out:
        return None
    return out


def l1b_expand_oof_val_windows() -> list[tuple[np.datetime64, np.datetime64]]:
    """L1b supervised expanding OOF when L1a features are in-matrix (fit pool starts at ``TRAIN_END``)."""
    pairs: list[tuple[np.datetime64, np.datetime64]] = []
    for a, b in L1B_EXPAND_OOF_VAL_WINDOWS:
        pairs.append((np.datetime64(str(a).strip()), np.datetime64(str(b).strip())))
    if not pairs:
        raise RuntimeError("L1B_EXPAND_OOF_VAL_WINDOWS is empty.")
    for vs, ve in pairs:
        if not (vs < ve):
            raise ValueError(f"Invalid L1b expanding OOF window {vs} .. {ve}")
    return pairs


def l1_expanding_oof_row_folds(
    time_key: pd.Series | np.ndarray,
    fit_pool: np.ndarray,
    *,
    val_windows: list[tuple[np.datetime64, np.datetime64]] | None = None,
) -> list[tuple[np.ndarray, np.ndarray]]:
    """Row-level (bar) masks inside ``fit_pool``: train rows t < vs; val rows vs <= t < ve."""
    ts = np.asarray(pd.to_datetime(time_key))
    pool = np.asarray(fit_pool, dtype=bool).ravel()
    wins = val_windows if val_windows is not None else l1_expand_oof_val_windows()
    folds: list[tuple[np.ndarray, np.ndarray]] = []
    for vs, ve in wins:
        tr = pool & (ts < vs)
        va = pool & (ts >= vs) & (ts < ve)
        folds.append((tr, va))
    return folds


def l1_expanding_oof_window_folds(
    time_at_end: np.ndarray,
    *,
    val_windows: list[tuple[np.datetime64, np.datetime64]] | None = None,
) -> list[tuple[np.ndarray, np.ndarray]]:
    """Window-axis masks (len n_windows): train if sequence end time < vs; val if vs <= end < ve."""
    t = np.asarray(pd.to_datetime(time_at_end))
    wins = val_windows if val_windows is not None else l1_expand_oof_val_windows()
    folds: list[tuple[np.ndarray, np.ndarray]] = []
    for vs, ve in wins:
        w_tr = t < vs
        w_va = (t >= vs) & (t < ve)
        folds.append((w_tr, w_va))
    return folds


def l2_oof_folds_from_env() -> int:
    """Layer-2 blocked time OOF fold count.

    Default ``5``: full calibration window is split into K contiguous time blocks;
    each block is validated once with models trained on the other blocks; final
    LGBM rounds = median best_iteration across folds; calibrators fit on stacked OOF preds.

    Set ``L2_OOF_FOLDS=1`` to restore the legacy ``L2_VAL_START`` time holdout inside cal.
    """
    raw = os.environ.get("L2_OOF_FOLDS", "5").strip() or "5"
    try:
        n = int(raw)
    except ValueError:
        n = 5
    return max(1, min(n, 128))


def l3_oof_folds_from_env() -> int:
    """Layer-3 blocked time OOF on the OOT policy window (see ``L3_OOT_START`` / ``L3_HOLDOUT_START``).

    Default ``5``: split OOT rows into K contiguous time blocks; ES per fold; final rounds = median
    ``best_iteration``; exit calibrator fits on stacked OOF raw probabilities on the full OOT fit pool.

    Set ``L3_OOF_FOLDS=1`` for the legacy trade-id70/30 train/val split inside OOT.
    """
    raw = os.environ.get("L3_OOF_FOLDS", "5").strip() or "5"
    try:
        n = int(raw)
    except ValueError:
        n = 5
    return max(1, min(n, 128))


def l1_oof_folds_from_env() -> int:
    """Layer-1 OOF fold count.

    Expanding mode: fixed by ``L1_EXPAND_OOF_VAL_WINDOWS`` (``L1_OOF_FOLDS`` ignored).

    Blocked mode: default ``3`` contiguous time blocks inside ``time_key < CAL_END``;
    ``L1_OOF_FOLDS=1`` disables OOF (legacy train vs ``l2_val`` only).
    """
    if l1_oof_mode_from_env() == "expanding":
        return len(l1_expand_oof_val_windows())
    raw = os.environ.get("L1_OOF_FOLDS", "3").strip() or "3"
    try:
        n = int(raw)
    except ValueError:
        n = 3
    return max(1, min(n, 128))


def split_mask_for_tuning_and_report(
    time_key: pd.Series | np.ndarray,
    base_mask: np.ndarray,
    *,
    tune_frac: float,
    min_rows_each: int = 50,
) -> tuple[np.ndarray, np.ndarray]:
    """Time-ordered split of ``base_mask`` into an early (tune) and late (report) slice."""
    base = np.asarray(base_mask, dtype=bool)
    idx = np.flatnonzero(base)
    tune_mask = np.zeros_like(base, dtype=bool)
    report_mask = np.zeros_like(base, dtype=bool)
    if idx.size == 0:
        return tune_mask, report_mask
    ts = np.asarray(pd.to_datetime(time_key))
    idx = idx[np.argsort(ts[idx], kind="mergesort")]
    if idx.size < 2 * min_rows_each:
        tune_mask[idx] = True
        report_mask[idx] = True
        return tune_mask, report_mask
    split = int(round(idx.size * float(np.clip(tune_frac, 0.2, 0.8))))
    split = max(min_rows_each, min(idx.size - min_rows_each, split))
    tune_mask[idx[:split]] = True
    report_mask[idx[split:]] = True
    return tune_mask, report_mask


def balanced_time_fold_chunk_sizes(n: int, k: int) -> list[int]:
    if k <= 0:
        return [n]
    q, r = divmod(n, k)
    return [q + 1] * r + [q] * (k - r)


def time_blocked_fold_masks(
    time_key: pd.Series | np.ndarray,
    base_mask: np.ndarray,
    n_folds: int,
    *,
    context: str = "OOF",
) -> list[tuple[np.ndarray, np.ndarray]]:
    """K disjoint contiguous time blocks inside ``base_mask``; each tuple is (train_bool, val_bool) for one fold."""
    base = np.asarray(base_mask, dtype=bool).ravel()
    n = base.size
    idx = np.flatnonzero(base)
    if idx.size < n_folds * 10:
        raise RuntimeError(
            f"{context}: need at least {n_folds * 10} rows in fit mask, got {idx.size} "
            f"(reduce fold count or widen the fit window)."
        )
    ts = np.asarray(pd.to_datetime(time_key))
    idx = idx[np.argsort(ts[idx], kind="mergesort")]
    sizes = balanced_time_fold_chunk_sizes(int(idx.size), n_folds)
    folds: list[tuple[np.ndarray, np.ndarray]] = []
    start = 0
    for sz in sizes:
        end = start + int(sz)
        va_idx = idx[start:end]
        tr_idx = np.setdiff1d(idx, va_idx, assume_unique=False)
        tr_m = np.zeros(n, dtype=bool)
        va_m = np.zeros(n, dtype=bool)
        tr_m[tr_idx] = True
        va_m[va_idx] = True
        folds.append((tr_m, va_m))
        start = end
    return folds


def build_stack_time_splits(time_key: pd.Series | np.ndarray) -> StackTimeSplits:
    ts = pd.to_datetime(np.asarray(time_key))
    train_mask = ts < np.datetime64(TRAIN_END)
    cal_mask = (ts >= np.datetime64(TRAIN_END)) & (ts < np.datetime64(CAL_END))
    test_mask = (ts >= np.datetime64(CAL_END)) & (ts < np.datetime64(TEST_END))

    if l1_oof_mode_from_env() == "expanding":
        wins = l1_expand_oof_val_windows()
        if str(wins[0][0]) != str(np.datetime64(TRAIN_END)):
            raise ValueError(
                f"TRAIN_END={TRAIN_END} must match first expanding val start {wins[0][0]} "
                "(L1 strict train is bars before first OOF val segment)."
            )
        if str(wins[-1][1]) != str(np.datetime64(CAL_END)):
            raise ValueError(
                f"CAL_END={CAL_END} must match end of last expanding val window {wins[-1][1]}."
            )
        l2_train_mask = np.zeros_like(ts, dtype=bool)
        for vs, ve in wins[:-1]:
            l2_train_mask |= (ts >= vs) & (ts < ve)
        vs_l, ve_l = wins[-1]
        l2_val_mask = (ts >= vs_l) & (ts < ve_l)
    else:
        l2_val_start = l2_val_start_time()
        if not (np.datetime64(TRAIN_END) < l2_val_start < np.datetime64(CAL_END)):
            raise ValueError(
                f"L2_VAL_START={str(l2_val_start)!r} must lie strictly inside calibration window "
                f"[{TRAIN_END}, {CAL_END})."
            )
        l2_train_mask = (ts >= np.datetime64(TRAIN_END)) & (ts < l2_val_start)
        l2_val_mask = (ts >= l2_val_start) & (ts < np.datetime64(CAL_END))

    if not l2_train_mask.any() or not l2_val_mask.any():
        raise RuntimeError(
            "L2 strict time split produced an empty train or val mask. "
            f"Check TRAIN_END={TRAIN_END}, CAL_END={CAL_END}, L1_OOF_MODE={l1_oof_mode_from_env()}."
        )

    return StackTimeSplits(
        train_mask=train_mask,
        cal_mask=cal_mask,
        test_mask=test_mask,
        l2_train_mask=l2_train_mask,
        l2_val_mask=l2_val_mask,
    )


def compute_transition_event_labels(state_label: np.ndarray, symbols: np.ndarray, *, horizon: int = 10) -> np.ndarray:
    labels = np.zeros(len(state_label), dtype=np.float32)
    state_label = np.asarray(state_label, dtype=np.int64)
    symbols = np.asarray(symbols)
    horizon = max(int(horizon), 1)
    for sym in np.unique(symbols):
        idx = np.flatnonzero(symbols == sym)
        s = state_label[idx]
        n = len(s)
        for i in range(n):
            future = s[i + 1 : min(n, i + 1 + horizon)]
            labels[idx[i]] = float(np.any(future != s[i])) if future.size else 0.0
    return labels


def l1a_transition_calibrate_enabled() -> bool:
    """When true (default), pick transition lookahead on fit_mask so positive rate is ~30–50% (tunable)."""
    v = (os.environ.get("L1A_TRANSITION_CALIBRATE", "1") or "1").strip().lower()
    return v in {"1", "true", "yes", "on"}


def _l1a_transition_pos_rate(
    state_label: np.ndarray,
    symbols: np.ndarray,
    fit_mask: np.ndarray,
    horizon: int,
) -> float:
    y = compute_transition_event_labels(state_label, symbols, horizon=max(1, int(horizon)))
    m = np.asarray(fit_mask, dtype=bool).ravel() & np.isfinite(y)
    return float(np.mean(y[m])) if m.any() else 0.0


def l1a_resolve_transition_horizon_bars(
    state_label: np.ndarray,
    symbols: np.ndarray,
    fit_mask: np.ndarray | None,
    *,
    forecast_horizon: int,
) -> tuple[int, dict[str, Any]]:
    """Choose lookahead H for transition BCE. Larger H → higher positive rate (monotone in typical data)."""
    state_label = np.asarray(state_label, dtype=np.int64)
    symbols = np.asarray(symbols)
    fit = np.ones(len(state_label), dtype=bool) if fit_mask is None else np.asarray(fit_mask, dtype=bool).ravel()
    fh = max(1, int(forecast_horizon))

    raw = os.environ.get("L1A_TRANSITION_HORIZON", "").strip()
    if raw:
        h = max(1, int(raw))
        r = _l1a_transition_pos_rate(state_label, symbols, fit, h)
        return h, {"mode": "env", "horizon": h, "pos_rate_fit": r, "forecast_horizon": fh}

    if not l1a_transition_calibrate_enabled():
        r = _l1a_transition_pos_rate(state_label, symbols, fit, fh)
        return fh, {"mode": "forecast_linked", "horizon": fh, "pos_rate_fit": r, "forecast_horizon": fh}

    lo = float(os.environ.get("L1A_TRANSITION_TARGET_POS_LO", "0.30"))
    hi = float(os.environ.get("L1A_TRANSITION_TARGET_POS_HI", "0.50"))
    h_min = max(1, int(os.environ.get("L1A_TRANSITION_H_MIN", "1")))
    h_max = max(h_min, int(os.environ.get("L1A_TRANSITION_H_MAX", "120")))

    in_band: list[tuple[int, float]] = []
    for h in range(h_min, h_max + 1):
        r = _l1a_transition_pos_rate(state_label, symbols, fit, h)
        if lo <= r <= hi:
            in_band.append((h, r))
    if in_band:
        h_pick, r_pick = min(in_band, key=lambda x: abs(x[0] - fh))
        return h_pick, {
            "mode": "calibrated",
            "horizon": h_pick,
            "pos_rate_fit": r_pick,
            "target_band": [lo, hi],
            "forecast_horizon": fh,
        }

    mid = 0.5 * (lo + hi)
    best_h, best_r = fh, _l1a_transition_pos_rate(state_label, symbols, fit, fh)
    best_dist = abs(best_r - mid)
    for h in range(h_min, h_max + 1):
        r = _l1a_transition_pos_rate(state_label, symbols, fit, h)
        d = abs(r - mid)
        if d < best_dist - 1e-9 or (abs(d - best_dist) < 1e-9 and abs(h - fh) < abs(best_h - fh)):
            best_dist = d
            best_h, best_r = h, r
    return best_h, {
        "mode": "calibrated_nearest",
        "horizon": best_h,
        "pos_rate_fit": best_r,
        "target_band": [lo, hi],
        "forecast_horizon": fh,
        "note": "no H in [L1A_TRANSITION_H_MIN,L1A_TRANSITION_H_MAX] hit band; picked rate closest to band midpoint",
    }


def log_label_baseline(head_name: str, y: np.ndarray, task: str = "auto") -> None:
    """Print compact label diagnostics before fitting a head."""
    y = np.asarray(y, dtype=np.float64).ravel()
    n = len(y)
    nan_n = int(np.isnan(y).sum())
    inf_n = int(np.isinf(y).sum())
    finite = y[np.isfinite(y)]
    if task == "auto":
        nuniq = len(np.unique(finite)) if finite.size else 0
        task = "cls" if nuniq <= 20 else "reg"
    print(f"  [label-baseline] {head_name}  n={n:,}  NaN={nan_n}  Inf={inf_n}  task={task}", flush=True)
    if finite.size == 0:
        print("    (skip: no finite labels)", flush=True)
        return
    if task == "cls":
        vals, counts = np.unique(finite, return_counts=True)
        total = int(counts.sum())
        for v, c in zip(vals.tolist(), counts.tolist()):
            pct = 100.0 * float(c) / max(total, 1)
            flag = "  <- skew" if pct > 95.0 or pct < 1.0 else ""
            print(f"    class={v:g}: {c:>8,} ({pct:5.1f}%){flag}", flush=True)
        if len(vals) < 2:
            print("    WARNING: single-class label; model cannot learn a separator.", flush=True)
        return
    pcts = np.percentile(finite, [1, 5, 25, 50, 75, 95, 99])
    print(
        f"    mean={float(np.mean(finite)):.6f}  std={float(np.std(finite)):.6f}  "
        f"min={float(np.min(finite)):.6f}  max={float(np.max(finite)):.6f}",
        flush=True,
    )
    print(f"    pcts[1/5/25/50/75/95/99]={np.round(pcts, 6).tolist()}", flush=True)
    nuniq = len(np.unique(np.round(finite, 6)))
    if nuniq <= 3:
        print(f"    WARNING: only {nuniq} unique rounded values; label may be degenerate.", flush=True)


def save_output_cache(df: pd.DataFrame, filename: str) -> str:
    os.makedirs(MODEL_DIR, exist_ok=True)
    path = os.path.join(MODEL_DIR, filename)
    with open(path, "wb") as f:
        pickle.dump(df, f)
    return path


def load_output_cache(filename: str) -> pd.DataFrame:
    path = os.path.join(MODEL_DIR, filename)
    with open(path, "rb") as f:
        obj = pickle.load(f)
    if not isinstance(obj, pd.DataFrame):
        raise TypeError(f"Cache {path} is not a pandas DataFrame.")
    return obj


def l2_l3_synthetic_decision_class(
    straddle_on: np.ndarray,
    l1b_edge: np.ndarray | None,
    gate_p: np.ndarray | None,
) -> np.ndarray:
    """L3 / OOS entry semantics: 0=long_vol, 1=no_trade, 2=short_vol.

    ``straddle_on`` is interpreted as a signed vol signal when available:
      - >0.5  => long_vol
      - <−0.5 => short_vol
      - else  => no_trade
    For legacy callers that still pass a binary straddle mask, only long_vol/no_trade
    will be emitted.
    """
    sig = np.asarray(straddle_on, dtype=np.float64).ravel()
    n = int(sig.size)
    out = np.ones(n, dtype=np.int32)
    if n == 0:
        return out
    out[sig > 0.5] = 0
    out[sig < -0.5] = 2
    return out


def l2_l3_entry_decision_class_from_merged(merged: pd.DataFrame) -> np.ndarray:
    """Build per-row {0,1,2} entry class; prefer explicit vol-signal outputs."""
    n = int(len(merged))
    if n == 0:
        return np.zeros(0, dtype=np.int64)
    if "l2_decision_class" in merged.columns:
        s = pd.to_numeric(merged["l2_decision_class"], errors="coerce").to_numpy(dtype=np.float64, copy=False)
        o = np.where(np.isfinite(s), s, 1.0)
        return np.clip(o, 0, 2).astype(np.int64)
    if "l2_vol_signal" in merged.columns:
        sig = pd.to_numeric(merged["l2_vol_signal"], errors="coerce").fillna(0.0).to_numpy(dtype=np.float64)
        return np.asarray(l2_l3_synthetic_decision_class(sig, None, None), dtype=np.int64)
    st = pd.to_numeric(merged.get("l2_straddle_on", 0), errors="coerce").fillna(0.0).to_numpy(dtype=np.float64)
    ge: np.ndarray | None = None
    if "l1b_edge_pred" in merged.columns:
        ge = pd.to_numeric(merged["l1b_edge_pred"], errors="coerce").fillna(0.0).to_numpy(dtype=np.float64)
    gp: np.ndarray | None = None
    for k in ("l2_gate_prob", "l2_decision_confidence"):
        if k in merged.columns:
            gp = pd.to_numeric(merged[k], errors="coerce").fillna(0.0).to_numpy(dtype=np.float64)
            break
    return np.asarray(l2_l3_synthetic_decision_class(st, ge, gp), dtype=np.int64)
