...
def _l3_policy_tqdm_file():
    """Stream for L3 bar-scan tqdm: same as :func:`_tqdm_stream` (real TTY or stderr), never the tee'ing ``Logger``/stdout, so progress bars stay out of ``logs/l2l3_unified.log``."""
    return _tqdm_stream()
...
def _decision_edge_atr_array(df: pd.DataFrame) -> np.ndarray:
    ...
    raise RuntimeError(
        ...
        "Regenerate labels with `archive/data_tools/label_v2.py` before training."
    )
...
def _l3_policy_dataset_tqdm_enabled() -> bool:
    """L3 bar-scan progress bar. On by default; independent of ``LGBM_DISABLE_ROUND_TQDM`` / ``FORCE_TQDM``.

    Tqdm is written to :func:`_l3_policy_tqdm_file` (console or stderr), not to the log file.
    If you use ``2>&1 | tee``, tqdm may still be duplicated into the file.
    """
    if os.environ.get("DISABLE_TQDM", "").strip().lower() in {"1", "true", "yes"}:
        return False
    return True
...
def _decision_forward_range_atr_array(df: pd.DataFrame) -> np.ndarray:
    ...
    if "decision_forward_range_atr" not in df.columns:
        raise RuntimeError(
            ...
            "Regenerate labels with `archive/data_tools/label_v2.py`."
        )
...
def _decision_forward_range_h_atr_array(df: pd.DataFrame, *, bars: int) -> np.ndarray:
    ...
    if col not in df.columns:
        raise RuntimeError(
            f"Missing `{col}` — re-run archive/data_tools/label_v2.py (multi-horizon columns) or disable L2_MULTI_HORIZON."
        )
...
def _decision_range_ttp90_norm_30_array(df: pd.DataFrame) -> np.ndarray:
    ...
    if col not in df.columns:
        raise RuntimeError(
            f"Missing `{col}` — re-run archive/data_tools/label_v2.py or disable L2_MULTI_HORIZON."
        )
    return pd.to_numeric(df[col], errors="coerce").to_numpy(dtype=np.float64, copy=False)
