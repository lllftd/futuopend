...
from core.training.common.constants import BO_FEAT_COLS, PA_CTX_FEATURES, PA_STRADDLE_FEATURES
...
L1A_PREFERRED_CORE: tuple[str, ...] = (
    ...
    "pa_vol_rvol",
    "pa_vol_momentum",
    # Volume / vol-term slope (also in PA_STRADDLE_FEATURES): ensures inclusion in L1a column set; channel order is not
    # structurally special for the TCN (conv mixes channels) — presence in the tensor matters more than position.
    "pa_volume_zscore",
    "pa_rv_term_slope",
    "pa_bull_pressure",
    "pa_bear_pressure",
...
def l1a_preferred_columns() -> tuple[str, ...]:
    ...
    if l1_ctx_stagger_enabled():
        parts = L1A_PREFERRED_CORE + L1A_CTX_DYNAMIC_COLUMNS + straddle
    else:
        parts = L1A_PREFERRED_CORE + L1A_CTX_LEGACY_COLUMNS + straddle
    seen: set[str] = set()
    ordered: list[str] = []
    for c in parts:
        if c not in seen:
