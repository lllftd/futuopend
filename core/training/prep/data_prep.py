...
from core.models.mamba_pa_state import PAStateMamba
from core.models.tcn_pa_state import PAStateTCN, FocalLoss
from core.training.prep.pa_feature_cache import load_or_build_pa_features
from core.training.prep.pa_state_controls import ensure_pa_state_features

from core.training.common.constants import *
from core.training.prep.feature_drift import (
    compute_train_window_stats,
    report_prep_drift_vs_saved,
    save_prep_fingerprint,
)
from core.training.prep.feature_registry import (
    describe_feature_coverage,
    parse_prep_layer_targets,
    prep_needs_mamba,
    prep_needs_tcn_derivatives,
)
from core.training.common.lgbm_utils import *
from core.training.tcn.tcn_constants import STATE_CLASSIFIER_FILE as TCN_STATE_DICT_BASENAME
...
def _compute_tcn_derived_features(df: pd.DataFrame, base_feat_cols: list[str]) -> pd.DataFrame:
    ...
    if not (os.path.exists(meta_path) and os.path.exists(model_path)):
        raise RuntimeError(
            ...
            "train_pipeline normally trains this automatically before the first prepare_dataset; "
            "if you call prepare_dataset directly, run ensure_tcn_state_classifier_checkpoint() from "
            "core.training.prep.tcn_data_prep, or restore the two files from a backup. "
            "To skip auto-train: TCN_SKIP_AUTO_TRAIN=1."
        )
    ...
    try:
        ...
        _bs_default = "8192" if device.type == "cuda" else ("4096" if device.type == "mps" else "64")
        ...
        if os.path.exists(oof_path):
            ...
            if "regime_probs" not in oof:
                raise RuntimeError(
                    f"{oof_path} is missing 'regime_probs' (transition). Retrain Layer 1 via ./scripts/training/run_train.sh layer1"
                )
            ...
            if oem.shape[1] != bottleneck_dim:
                raise RuntimeError(
                    ...
                    f"from tcn_meta.pkl. Retrain Layer 1 via ./scripts/training/run_train.sh layer1"
                )
            for j in range(bottleneck_dim):
...
def _compute_mamba_derived_features(df: pd.DataFrame, base_feat_cols: list[str]) -> pd.DataFrame:
    ...
    if not (os.path.exists(meta_path) and os.path.exists(model_path)):
        raise RuntimeError(
            ...
            "(from Layer 1 / backtests.train_pipeline). Retrain: ./scripts/training/run_train.sh layer1"
        )
        ...
        if os.path.exists(oof_path):
            ...
            if oem.shape[1] != bottleneck_dim:
                raise RuntimeError(
                    ...
                    f"from mamba_meta.pkl. Retrain Layer 1 via ./scripts/training/run_train.sh layer1"
                )
            for j in range(bottleneck_dim):
...
def prepare_dataset(symbols: list[str] = ["QQQ", "SPY"]):
    ...
    prep_targets = parse_prep_layer_targets()
    print(f"  PREPARED_DATASET_LAYER_TARGETS -> {sorted(prep_targets)}", flush=True)
    if os.environ.get("PREP_PRINT_FEATURE_COVERAGE", "").strip().lower() in {"1", "true", "yes"}:
        print(describe_feature_coverage(), flush=True)
    if os.environ.get("PREP_FEATURE_DRIFT_CHECK", "1").strip().lower() not in {
        "0",
        "false",
        "no",
        "off",
    ...
    # Six-class regime mode (A+B imputation):
    #   A) posterior impute from pa_hmm_prob_* (argmax)
    #   B) per-symbol Markov smoothing over remaining gaps
    #   hard fail if still missing after A+B.
    hmm_state = pd.to_numeric(df["pa_hmm_state"], errors="coerce")
    state_raw = hmm_state.fillna(ms_num).astype("float64")
    missing0 = state_raw.isna()
    n_missing0 = int(missing0.sum())
    n_fill_a = 0
    n_fill_b = 0
    ...
    if n_missing0:
        ...
        if has_probs:
            ...
            valid_a = missing0 & probs.notna().all(axis=1) & np.isfinite(row_sum) & (row_sum > 0.0)
            if valid_a.any():
                a_idx = probs.loc[valid_a].to_numpy(dtype=np.float64).argmax(axis=1).astype(np.int64)
                state_raw.loc[valid_a] = a_idx
                n_fill_a = int(valid_a.sum())

    # B) Markov smoothing for residual missing states.
    missing1 = state_raw.isna()
    if missing1.any():
        for sym, idx in df.groupby("symbol", sort=False).groups.items():
            idx = np.asarray(idx, dtype=np.int64)
            arr = state_raw.iloc[idx].to_numpy(dtype=np.float64)
            ...
            miss_pos = np.where(miss)[0]
            for p in miss_pos:
                prev = p - 1
                while prev >= 0 and not np.isfinite(arr[prev]):
                    prev -= 1
                nxt = p + 1
                while nxt < len(arr) and not np.isfinite(arr[nxt]):
                    nxt += 1

                if prev >= 0 and nxt < len(arr):
                    s_prev = int(arr[prev])
                    s_next = int(arr[nxt])
                    score = trans[s_prev, :] * trans[:, s_next]
                elif prev >= 0:
                    s_prev = int(arr[prev])
                    score = trans[s_prev, :]
                elif nxt < len(arr):
                    ...
                    score = prior
                arr[p] = float(int(np.argmax(score)))
                n_fill_b += 1

            state_raw.iloc[idx] = arr

    missing2 = state_raw.isna()
    if missing2.any():
        bad = df.loc[missing2, ["symbol", "time_key", "market_state", "pa_hmm_state"]].head(20)
        raise RuntimeError(
            "state_label imputation failed after A+B; still missing rows="
    ...
    if os.environ.get("PREP_SAVE_FINGERPRINT", "1").strip().lower() not in {"0", "false", "no", "off"}:
        ...
        fp_path = save_prep_fingerprint(feat_cols, fp_stats, fp_n)
        print(
            f"  Train-window feature fingerprint: {len(fp_stats)} cols, n={fp_n:,} -> {fp_path}",
            flush=True,
        )
