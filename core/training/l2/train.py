...
from core.training.l2.calibration import (
...
def _l2_multi_horizon_enabled(df: pd.DataFrame) -> bool:
    ...
    if mode in {"1", "true", "yes", "on"} and not has_all:
        raise RuntimeError(
            ...
            "Re-run archive/data_tools/label_v2.py and PREPARED_DATASET_CACHE_REBUILD=1."
        )
    return bool(has_all)
...
L2_OUTPUT_COLS = [
    ...
    "l2_range_pred",
    "l2_realized_pred_range",
    "l2_implied_proxy_range",
    "l2_vol_edge",
    "l2_range_pred_10",
    "l2_range_pred_20",
    "l2_pred_ttp90_norm",
...
def _l2_iv_proxy_rv_lookback() -> int:
    return int(np.clip(int(L2_VOL_ONLY_DEFAULTS.get("iv_proxy_rv_lookback", 390)), 30, 5000))


def _l2_iv_proxy_vixy_z_beta() -> float:
    return float(np.clip(float(L2_VOL_ONLY_DEFAULTS.get("iv_proxy_vixy_z_beta", 0.05)), -1.0, 1.0))


def _l2_rv_to_iv_mult() -> float:
    d = L2_VOL_ONLY_DEFAULTS
    raw = d.get("rv_to_iv_mult", d.get("vrp_base_mult", 1.0 / 0.85))
    return float(np.clip(float(raw), 0.5, 3.0))


def _l2_implied_range_sigma_mult() -> float:
    return float(np.clip(float(L2_VOL_ONLY_DEFAULTS.get("implied_range_sigma_mult", 1.0)), 0.1, 5.0))


def _l2_vol_long_edge_thr() -> float:
    return float(np.clip(float(L2_VOL_ONLY_DEFAULTS.get("long_edge_thr", 0.20)), 0.0, 5.0))


def _l2_vol_short_edge_thr() -> float:
    return float(np.clip(float(L2_VOL_ONLY_DEFAULTS.get("short_edge_thr", 0.20)), 0.0, 5.0))


def _l2_implied_proxy_range_atr(
    df: pd.DataFrame,
    frame: pd.DataFrame,
    *,
    hold_bars: int,
    rv_lookback: int | None = None,
    vixy_z_beta: float | None = None,
    vrp_base_mult: float | None = None,
    implied_range_sigma_mult: float | None = None,
) -> np.ndarray:
    """Approx implied forward range in **the same ATR units** as ``decision_forward_range_atr``.

    Labels use ``(max(high)-min(low)) / atr`` where ``atr`` is written as ``lbl_atr`` after prep
    (``archive/data_tools/label_v2.py`` names it ``atr`` before rename). This uses ``lbl_atr`` via
    ``_l2_bracket_atr_array`` so ``range_pred`` and ``implied_proxy`` are comparable.
    """
    n = len(df)
    if n == 0:
    ...
    if "lbl_atr" not in df.columns:
        raise RuntimeError(
            "L2 vol edge requires column ``lbl_atr`` (same ATR as decision_forward_range_atr labels). "
            "Rebuild labels/prep so lbl_atr is present."
        )
    close = pd.to_numeric(df.get("close"), errors="coerce").fillna(method="ffill").fillna(method="bfill").fillna(1.0)
    close_arr = np.maximum(close.to_numpy(dtype=np.float64, copy=False), 1e-6)
    if "symbol" in df.columns:
        sym = df["symbol"].astype(str)
        ret = close.groupby(sym, sort=False).pct_change().fillna(0.0)
    else:
        ret = close.pct_change().fillna(0.0)
    lookback = int(rv_lookback if rv_lookback is not None else _l2_iv_proxy_rv_lookback())
    minp = max(20, lookback // 5)
    if "symbol" in df.columns:
        rv = ret.groupby(df["symbol"].astype(str), sort=False).transform(
            lambda s: s.rolling(lookback, min_periods=minp).std()
        )
    else:
        rv = ret.rolling(lookback, min_periods=minp).std()
    rv = rv.fillna(method="ffill").fillna(method="bfill").fillna(float(np.nanmedian(np.abs(ret))) if len(ret) else 0.0)
    rv_arr = np.clip(rv.to_numpy(dtype=np.float64, copy=False), 1e-7, 1.0)
    vrp_mult = float(vrp_base_mult if vrp_base_mult is not None else _l2_rv_to_iv_mult())
    vixy_z = (
        pd.to_numeric(frame["vixy_zscore_390"], errors="coerce").fillna(0.0).to_numpy(dtype=np.float64, copy=False)
        if "vixy_zscore_390" in frame.columns
        else np.zeros(n, dtype=np.float64)
    )
    beta = float(vixy_z_beta if vixy_z_beta is not None else _l2_iv_proxy_vixy_z_beta())
    z_adj = np.clip(1.0 + beta * vixy_z, 0.50, 1.50)
    sigma_hold = rv_arr * vrp_mult * np.sqrt(max(int(hold_bars), 1))
    range_mult = float(
        implied_range_sigma_mult if implied_range_sigma_mult is not None else _l2_implied_range_sigma_mult()
    )
    implied_range_pct = np.clip(range_mult * sigma_hold * z_adj, 1e-7, 2.5)
    atr = _l2_bracket_atr_array(df).astype(np.float64, copy=False)
    implied_atr = np.clip(implied_range_pct * close_arr / np.maximum(atr, 1e-6), 0.01, 50.0)
    return implied_atr.astype(np.float32, copy=False)


def _l2_vol_signal_from_edge(
    gate_p: np.ndarray,
    gate_thr: float | np.ndarray,
    vol_edge: np.ndarray,
    *,
    long_thr: float,
    short_thr: float,
) -> np.ndarray:
    g = np.asarray(gate_p, dtype=np.float64).ravel()
    e = np.asarray(vol_edge, dtype=np.float64).ravel()
    thr = np.asarray(gate_thr, dtype=np.float64).ravel()
    if thr.size == 1:
        thr = np.full_like(g, float(thr[0]), dtype=np.float64)
    elif thr.size != g.size:
        raise ValueError("gate threshold length mismatch in vol signal builder.")
    out = np.zeros(len(g), dtype=np.int8)
    active = g >= thr
    out[active & (e >= float(long_thr))] = 1
    out[active & (e <= -float(short_thr))] = -1
    return out
...
def _l2_parse_float_range_env(name: str, default: tuple[float, float]) -> tuple[float, float]:
    raw = os.environ.get(name, f"{default[0]},{default[1]}").strip()
    ...
    try:
        ...
        return default
    lo = float(np.clip(lo, 0.0, 5.0))
    hi = float(np.clip(hi, 0.0, 5.0))
    if hi < lo:
        lo, hi = hi, lo
    if abs(hi - lo) < 1e-12:
        hi = min(5.0, lo + 1e-6)
...
def _l2_direction_threshold_score(
    pnl: np.ndarray,
    trade_rate: float,
    *,
    target_trade_rate: float,
    short_pnl: np.ndarray | None = None,
    balance_ratio: float = 0.0,
    balance_bonus: float = 0.0,
    short_tail_mult: float = 1.0,
) -> tuple[float, dict[str, float]]:
    pr = np.asarray(pnl, dtype=np.float64).ravel()
    if pr.size == 0:
        return -1e18, {
            "mean_pnl": float("nan"),
            "downside_rms": float("nan"),
    ...
    eq = np.cumsum(pr)
    peak = np.maximum.accumulate(eq)
    max_dd = float(np.max(peak - eq)) if eq.size else 0.0
    p05 = float(np.quantile(pr, 0.05)) if pr.size else 0.0
    tail_loss = float(max(0.0, -p05))
    if short_pnl is not None:
        sp = np.asarray(short_pnl, dtype=np.float64).ravel()
        if sp.size:
            sp05 = float(np.quantile(sp, 0.05))
            short_tail_loss = float(max(0.0, -sp05))
            tail_loss += max(0.0, short_tail_mult - 1.0) * short_tail_loss
    gross_pos = float(np.sum(pr[pr > 0.0]))
    gross_neg = float(np.sum(-pr[pr < 0.0]))
    profit_factor = gross_pos / max(gross_neg, 1e-9)
    score = (
        ...
        - dd_pen * max_dd
        - tail_pen * tail_loss
        + pf_bonus * np.clip(profit_factor, 0.0, 5.0)
        - rate_pen * abs(float(trade_rate) - float(target_trade_rate))
        + float(balance_bonus) * float(np.clip(balance_ratio, 0.0, 1.0))
    )
    return float(score), {
        "mean_pnl": mean_p,
        "downside_rms": dstd,
...
def _search_l2_direction_thresholds(
    *,
    gate_p: np.ndarray,
    trade_threshold: float | np.ndarray,
    vol_edge: np.ndarray,
    realized_range_atr: np.ndarray,
    implied_proxy_range_atr: np.ndarray,
    tune_mask: np.ndarray,
    default_long_thr: float,
    default_short_thr: float,
    target_trade_rate: float,
) -> tuple[float, float, dict[str, float | int | str | bool]]:
    gp = np.asarray(gate_p, dtype=np.float64).ravel()
    ve = np.asarray(vol_edge, dtype=np.float64).ravel()
    rr = np.asarray(realized_range_atr, dtype=np.float64).ravel()
    ip = np.asarray(implied_proxy_range_atr, dtype=np.float64).ravel()
    m = np.asarray(tune_mask, dtype=bool).ravel()
    thr = np.asarray(trade_threshold, dtype=np.float64)
    if thr.ndim == 0:
        thr_arr = np.full(gp.shape, float(np.clip(float(thr), 0.0, 1.0)), dtype=np.float64)
    else:
        thr_arr = np.broadcast_to(np.clip(thr.reshape(-1), 0.0, 1.0), gp.shape)
    valid = m & np.isfinite(gp) & np.isfinite(ve) & np.isfinite(rr) & np.isfinite(ip)
    if not np.any(valid):
        return float(default_long_thr), float(default_short_thr), {
            "enabled": False,
            "fallback_reason": "empty_val_tune",
        }
    band_lo, band_hi = _l2_trade_rate_search_min_max()
    long_lo, long_hi = _l2_parse_float_range_env("L2_DIR_THR_LONG_RANGE", (0.005, 0.30))
    short_lo, short_hi = _l2_parse_float_range_env("L2_DIR_THR_SHORT_RANGE", (0.02, 0.30))
    grid_n = int(np.clip(int(os.environ.get("L2_DIR_THR_GRID_SIZE", "20")), 2, 100))
    min_long = int(np.clip(int(os.environ.get("L2_DIR_MIN_LONG_COUNT", "50")), 0, 1_000_000))
    min_short = int(np.clip(int(os.environ.get("L2_DIR_MIN_SHORT_COUNT", "30")), 0, 1_000_000))
    balance_bonus = float(np.clip(float(os.environ.get("L2_DIR_BALANCE_BONUS", "0.15")), 0.0, 10.0))
    short_tail_mult = float(np.clip(float(os.environ.get("L2_DIR_SHORT_TAIL_MULT", "1.5")), 0.0, 10.0))
    pnl_cost = float(np.clip(float(os.environ.get("L2_DIR_PNL_COST_ATR", "0.0")), 0.0, 20.0))
    long_grid = np.linspace(long_lo, long_hi, grid_n)
    short_grid = np.linspace(short_lo, short_hi, grid_n)
    gate_pass = gp >= thr_arr
    base = valid & gate_pass
    n_den = int(np.sum(valid))

    best: dict[str, float] | None = None

    def _eval(l_thr: float, s_thr: float, *, enforce_constraints: bool) -> dict[str, float] | None:
        long_mask = base & (ve > float(l_thr))
        short_mask = base & (ve < -float(s_thr))
        n_long = int(np.sum(long_mask))
        n_short = int(np.sum(short_mask))
        n_trade = n_long + n_short
        trade_rate = float(n_trade / max(n_den, 1))
        if enforce_constraints:
            if trade_rate < float(band_lo) or trade_rate > float(band_hi):
                return None
            if n_long < min_long or n_short < min_short:
                return None
        long_pnl = (rr - ip - pnl_cost)[long_mask]
        short_pnl = (ip - rr - pnl_cost)[short_mask]
        if n_trade <= 0:
            return None
        all_pnl = np.concatenate([long_pnl, short_pnl])
        balance_ratio = float(min(n_long / max(n_short, 1), n_short / max(n_long, 1))) if n_long and n_short else 0.0
        score, stats = _l2_direction_threshold_score(
            all_pnl,
            trade_rate,
            ...
            short_tail_mult=short_tail_mult,
        )
        return {
            "long_thr": float(l_thr),
            "short_thr": float(s_thr),
            "score": float(score),
            "long_n": float(n_long),
            "short_n": float(n_short),
            "trade_rate": float(trade_rate),
            "long_mean_pnl": float(np.mean(long_pnl)) if n_long else float("nan"),
            "short_mean_pnl": float(np.mean(short_pnl)) if n_short else float("nan"),
            "balance_ratio": balance_ratio,
            **stats,
        }

    for l_thr in long_grid:
        for s_thr in short_grid:
            rec = _eval(float(l_thr), float(s_thr), enforce_constraints=True)
            if rec is None:
                continue
            if best is None or float(rec["score"]) > float(best["score"]):
                best = rec

    fallback_reason = ""
    if best is None:
        fallback_reason = "no_grid_candidate_satisfied_constraints"
        best = _eval(float(default_long_thr), float(default_short_thr), enforce_constraints=False)
    if best is None:
        return float(default_long_thr), float(default_short_thr), {
            "enabled": False,
            "fallback_reason": fallback_reason or "empty_default_candidate",
        }

    def _sensitivity(axis: str, delta: float = 0.02) -> tuple[list[float], bool]:
        vals: list[float] = []
        for off in (-delta, 0.0, delta):
            lt = float(best["long_thr"] + (off if axis == "long" else 0.0))
            st = float(best["short_thr"] + (off if axis == "short" else 0.0))
            lt = float(np.clip(lt, long_lo, long_hi))
            st = float(np.clip(st, short_lo, short_hi))
            rec = _eval(lt, st, enforce_constraints=False)
            vals.append(float(rec["score"]) if rec is not None else float("nan"))
        center = vals[1]
        finite = np.asarray([v for v in vals if np.isfinite(v)], dtype=np.float64)
        unstable = False
        if np.isfinite(center) and abs(center) > 1e-9 and finite.size:
            unstable = bool(np.max(np.abs(finite - center)) / max(abs(center), 1e-9) > 0.20)
        return vals, unstable

    sens_long, unstable_long = _sensitivity("long")
    sens_short, unstable_short = _sensitivity("short")
    asymmetric = abs(float(best["long_thr"]) - float(best["short_thr"])) > 1e-9
    stable = not (unstable_long or unstable_short)
    combos = int(grid_n * grid_n)
    long_at_boundary = bool(
        abs(float(best["long_thr"]) - float(long_lo)) <= 1e-12
        or abs(float(best["long_thr"]) - float(long_hi)) <= 1e-12
    )
    short_at_boundary = bool(
        abs(float(best["short_thr"]) - float(short_lo)) <= 1e-12
        or abs(float(best["short_thr"]) - float(short_hi)) <= 1e-12
    )
    rate_at_boundary = bool(
        abs(float(best["trade_rate"]) - float(band_lo)) <= 1e-4
        or abs(float(best["trade_rate"]) - float(band_hi)) <= 1e-4
    )
    print(
        "\n  [L2] direction threshold search (2D grid on val_tune)\n"
        f"    grid: long=[{long_lo:.3f}..{long_hi:.2f}]x{grid_n}  "
        f"short=[{short_lo:.2f}..{short_hi:.2f}]x{grid_n}  combos={combos}\n"
        f"    constraints: min_long={min_long}  min_short={min_short}  rate_band=[{float(band_lo):.3f}, {float(band_hi):.3f}]\n"
        "    note: combined_pf/mean_pnl are val_tune search diagnostics using realized range, not expected live PnL.",
        flush=True,
    )
    ratio = float(best["long_n"] / max(float(best["short_n"]), 1.0))
    status = "stable" if stable else "UNSTABLE"
    print(
        f"  [L2] selected long_edge_thr={float(best['long_thr']):.3f}  "
        f"short_edge_thr={float(best['short_thr']):.3f}  score={float(best['score']):.6f}\n"
        f"    long_n={int(best['long_n'])}  short_n={int(best['short_n'])}  "
        f"ratio={ratio:.2f}:1  trade_rate={float(best['trade_rate']):.3f}\n"
        f"    long_mean_pnl={float(best['long_mean_pnl']):+.4f}  "
        f"short_mean_pnl={float(best['short_mean_pnl']):+.4f}  "
        f"combined_pf={float(best['profit_factor']):.3f}  asymmetric={asymmetric}",
        flush=True,
    )
    if long_at_boundary or short_at_boundary or rate_at_boundary:
        print(
            "  [L2] direction threshold boundary note: "
            f"long_at_boundary={long_at_boundary} short_at_boundary={short_at_boundary} "
            f"rate_at_boundary={rate_at_boundary}; inspect grid/rate-band before treating thresholds as settled.",
            flush=True,
        )
    print(
        "  [L2] direction threshold sensitivity:\n"
        f"    long_thr +/-0.02:  score=[{sens_long[0]:.6f}, {sens_long[1]:.6f}, {sens_long[2]:.6f}]\n"
        f"    short_thr +/-0.02: score=[{sens_short[0]:.6f}, {sens_short[1]:.6f}, {sens_short[2]:.6f}]  {status}",
        flush=True,
    )
    meta: dict[str, float | int | str | bool] = {
        "enabled": True,
        "fallback_reason": fallback_reason,
        "long_thr": float(best["long_thr"]),
        "short_thr": float(best["short_thr"]),
        "score": float(best["score"]),
        "long_n": int(best["long_n"]),
        "short_n": int(best["short_n"]),
        "trade_rate": float(best["trade_rate"]),
        "long_mean_pnl": float(best["long_mean_pnl"]),
        "short_mean_pnl": float(best["short_mean_pnl"]),
        "combined_pf": float(best["profit_factor"]),
        "balance_ratio": float(best["balance_ratio"]),
        "asymmetric": bool(asymmetric),
        "stable": bool(stable),
        "long_at_boundary": bool(long_at_boundary),
        "short_at_boundary": bool(short_at_boundary),
        "rate_at_boundary": bool(rate_at_boundary),
        "diagnostic_pnl_warning": "combined_pf/mean_pnl use realized val_tune range for threshold ranking only; not live expected PnL",
        "long_range_min": float(long_lo),
        "long_range_max": float(long_hi),
        "short_range_min": float(short_lo),
        "short_range_max": float(short_hi),
        "long_sensitivity_minus": float(sens_long[0]),
        "long_sensitivity_center": float(sens_long[1]),
        "long_sensitivity_plus": float(sens_long[2]),
        "short_sensitivity_minus": float(sens_short[0]),
        "short_sensitivity_center": float(sens_short[1]),
        "short_sensitivity_plus": float(sens_short[2]),
        "grid_size": int(grid_n),
        "candidate_count": int(combos),
        "min_long_count": int(min_long),
        "min_short_count": int(min_short),
        "balance_bonus": float(balance_bonus),
        "short_tail_mult": float(short_tail_mult),
        "pnl_cost_atr": float(pnl_cost),
        "rate_band_min": float(band_lo),
        "rate_band_max": float(band_hi),
    }
    return float(best["long_thr"]), float(best["short_thr"]), meta
...
def _l2_boost_rounds() -> int:
    default = 250 if FAST_TRAIN_MODE else 2000
...
def _derived_l2_feature_frame(
    ...
    *,
    ...
    if "pa_rv_cc_20" in merged.columns:
        rv_base = pd.to_numeric(merged["pa_rv_cc_20"], errors="coerce")
    elif price_df is not None and len(price_df) == len(merged) and {"close", "symbol"}.issubset(price_df.columns):
        close_px = pd.to_numeric(price_df["close"], errors="coerce").ffill().bfill()
        px_ret = close_px.groupby(price_df["symbol"].astype(str), sort=False).pct_change().fillna(0.0)
        rv_base = px_ret.groupby(price_df["symbol"].astype(str), sort=False).transform(
            lambda s: s.rolling(60, min_periods=20).std()
        ) * np.sqrt(390.0)
    elif price_df is not None and len(price_df) == len(merged) and "close" in price_df.columns:
        close_px = pd.to_numeric(price_df["close"], errors="coerce").ffill().bfill()
        rv_base = close_px.pct_change().fillna(0.0).rolling(60, min_periods=20).std() * np.sqrt(390.0)
    else:
        rv_base = pd.Series(np.nan, index=merged.index, dtype=np.float64)
    rv_base = pd.Series(rv_base, index=merged.index, dtype=np.float64).replace([np.inf, -np.inf], np.nan).ffill().bfill()
    rv_base = rv_base.fillna(float(np.nanmedian(rv_base.to_numpy(dtype=np.float64))) if rv_base.notna().any() else 0.0)
    rv_base = rv_base.clip(lower=1e-8)

    if "vixy_rvol_60" in merged.columns:
        iv_proxy = pd.to_numeric(merged["vixy_rvol_60"], errors="coerce")
    elif "vixy_abs_ret_60" in merged.columns:
        iv_proxy = pd.to_numeric(merged["vixy_abs_ret_60"], errors="coerce") * np.sqrt(390.0 / 60.0)
    elif "vixy_zscore_390" in merged.columns:
        # Last-resort positive proxy; prefer realized VIXY vol columns when available.
        iv_proxy = 1.0 + pd.to_numeric(merged["vixy_zscore_390"], errors="coerce").clip(-0.95, 8.0)
    else:
        iv_proxy = pd.Series(np.nan, index=merged.index, dtype=np.float64)
    iv_proxy = pd.Series(iv_proxy, index=merged.index, dtype=np.float64).replace([np.inf, -np.inf], np.nan).ffill().bfill()
    iv_proxy = iv_proxy.fillna(float(np.nanmedian(iv_proxy.to_numpy(dtype=np.float64))) if iv_proxy.notna().any() else 1.0)
    iv_proxy = iv_proxy.clip(lower=1e-8)

    def _ewm_z(s: pd.Series, *, span: int = 180) -> pd.Series:
        def _one(x: pd.Series) -> pd.Series:
            mu = x.ewm(span=span, min_periods=max(20, span // 6), adjust=False).mean()
            sd = x.ewm(span=span, min_periods=max(20, span // 6), adjust=False).std().clip(lower=1e-8)
            return (x - mu) / sd

        if "symbol" in merged.columns:
            return s.groupby(merged["symbol"].astype(str), sort=False).transform(_one)
        return _one(s)

    rv_z = _ewm_z(rv_base).replace([np.inf, -np.inf], np.nan).fillna(0.0).clip(-5.0, 5.0)
    iv_z = _ewm_z(iv_proxy).replace([np.inf, -np.inf], np.nan).fillna(0.0).clip(-5.0, 5.0)
    out["rv_iv_spread_z_60"] = (rv_z - iv_z).clip(-5.0, 5.0).astype(np.float32)

    log_rv = np.log(rv_base.clip(lower=1e-8))
    log_iv = np.log(iv_proxy.clip(lower=1e-8))
    if "symbol" in merged.columns:
        sym_key = merged["symbol"].astype(str)
        rv_mom = log_rv.groupby(sym_key, sort=False).diff(30)
        iv_mom = log_iv.groupby(sym_key, sort=False).diff(30)
        log_ratio = (log_rv - log_iv).groupby(sym_key, sort=False).diff(15)
    else:
        rv_mom = log_rv.diff(30)
        iv_mom = log_iv.diff(30)
        log_ratio = (log_rv - log_iv).diff(15)
    out["vol_momentum_diff_60"] = (rv_mom - iv_mom).replace([np.inf, -np.inf], np.nan).fillna(0.0).clip(-3.0, 3.0).astype(np.float32)
    out["rv_iv_ratio_change_60"] = log_ratio.replace([np.inf, -np.inf], np.nan).fillna(0.0).clip(-3.0, 3.0).astype(np.float32)

    for _c in ("rv_iv_spread_z_60", "vol_momentum_diff_60", "rv_iv_ratio_change_60"):
        _s = pd.to_numeric(out[_c], errors="coerce")
        print(
            f"  [L2] core vol feature {_c}: nan={float(_s.isna().mean()):.3f} "
            f"nunique={int(_s.nunique(dropna=False))} min={float(_s.min()):.6f} "
            f"max={float(_s.max()):.6f} std={float(_s.std()):.6f}",
            flush=True,
        )

    out["qqq_spy_corr_60"] = np.zeros(n_m, dtype=np.float32)
    out["qqq_spy_vol_ratio_60"] = np.ones(n_m, dtype=np.float32)
    out["qqq_beta_60"] = np.ones(n_m, dtype=np.float32)
    if (
        price_df is not None
        and len(price_df) == len(merged)
        and {"time_key", "symbol", "close"}.issubset(price_df.columns)
    ):
        px = price_df[["time_key", "symbol", "close"]].copy()
        px["time_key"] = pd.to_datetime(px["time_key"], errors="coerce")
        px["symbol"] = px["symbol"].astype(str)
        px["close"] = pd.to_numeric(px["close"], errors="coerce")
        piv = px.pivot_table(index="time_key", columns="symbol", values="close", aggfunc="last")
        if "QQQ" in piv.columns and "SPY" in piv.columns:
            qqq_ret = piv["QQQ"].pct_change()
            spy_ret = piv["SPY"].pct_change()
            corr = qqq_ret.rolling(60, min_periods=20).corr(spy_ret)
            qv = qqq_ret.rolling(60, min_periods=20).std()
            sv = spy_ret.rolling(60, min_periods=20).std()
            cov = qqq_ret.rolling(60, min_periods=20).cov(spy_ret)
            var = spy_ret.rolling(60, min_periods=20).var()
            cross = pd.DataFrame(
                {
                    "qqq_spy_corr_60": corr,
                    "qqq_spy_vol_ratio_60": (qv / (sv + 1e-8)),
                    "qqq_beta_60": (cov / (var + 1e-8)),
                }
            ).replace([np.inf, -np.inf], np.nan)
            aligned = cross.reindex(pd.to_datetime(merged["time_key"], errors="coerce")).fillna(method="ffill").fillna(0.0)
            out["qqq_spy_corr_60"] = aligned["qqq_spy_corr_60"].clip(-1.0, 1.0).to_numpy(dtype=np.float32, copy=False)
            out["qqq_spy_vol_ratio_60"] = aligned["qqq_spy_vol_ratio_60"].clip(0.0, 5.0).to_numpy(dtype=np.float32, copy=False)
            out["qqq_beta_60"] = aligned["qqq_beta_60"].clip(-5.0, 5.0).to_numpy(dtype=np.float32, copy=False)
    return out
...
_L2_PRE_FILTER_PROTECT: frozenset[str] = frozenset(
    {
        "symbol",
        "time_key",
        "qqq_spy_corr_60",
        "qqq_spy_vol_ratio_60",
        "qqq_beta_60",
    }
)
...
def train_l2_trade_decision(
    ...
    mispricing_cost_atr = float(
        np.clip(float(os.environ.get("L2_MISPRICING_GATE_COST_ATR", "2.0")), 0.0, 20.0)
    )
    range_horizon_meta = _l2_range_horizon_bars()
    label_cost_mode = os.environ.get("L2_LABEL_COST_MODE", "fixed").strip().lower()
    ...
    if range_roll_cfg is not None:
    ...
    elif label_cost_mode in {"implied", "implied_proxy", "iv_proxy"}:
        dyn_cfg = None
        cost_atr_row = _l2_implied_proxy_range_atr(
            df,
            frame,
            hold_bars=range_horizon_meta,
        ...
        straddle_cost_mode = "implied_proxy"
    elif label_cost_mode in {"implied_proxy_min", "iv_proxy_min"}:
        dyn_cfg = None
        implied_cost = _l2_implied_proxy_range_atr(
            df,
            frame,
            hold_bars=range_horizon_meta,
            rv_lookback=_l2_iv_proxy_rv_lookback(),
        ...
        cost_atr_row = np.minimum(np.full(len(df), float(cost_atr_base), dtype=np.float64), implied_cost)
        straddle_cost_mode = "implied_proxy_min"
    else:
        dyn_cfg = None
    ...
    if mh_on:
        ...
        implied_proxy_label_atr = _l2_implied_proxy_range_atr(
            df,
            frame,
            hold_bars=range_horizon_meta,
            rv_lookback=_l2_iv_proxy_rv_lookback(),
        ...
        realized_abs_mispricing_atr = np.abs(realized_range_atr.astype(np.float64) - implied_proxy_label_atr)
        y_trade = (realized_abs_mispricing_atr > mispricing_cost_atr).astype(np.int64)
        y_range = realized_range_atr.astype(np.float32)
    ...
    straddle_label_stats = {
        ...
        "mispricing_cost_atr": float(mispricing_cost_atr),
        ...
        "multi_horizon_label_rule": (
            ...
            if mh_on
            else "trade_gate = (abs(forward_range_horizon - implied_proxy_range) > mispricing_cost); primary range target uses configured horizon"
        ),
        "mh_costs_atr": {"10": c10, "20": c20, "30": c30} if mh_on else None,
    }
    _prep_tick("labels_targets")

    from core.training.unified.train import L2PytorchContext, train_l2_pytorch_unified

    prep_bar.close()
    return train_l2_pytorch_unified(
        L2PytorchContext(
            train_started_at=train_started_at,
            train_started_perf=train_started_perf,
            df=df,
            l1a_outputs=l1a_outputs,
            l1b_outputs=l1b_outputs,
            frame=frame,
            feature_cols=feature_cols,
            range_feature_cols=range_feature_cols,
            X=X,
            X_range=X_range,
            X_train_fit=X_train_fit,
            X_range_train_fit=X_range_train_fit,
            y_trade=y_trade,
            y_range=y_range,
            y_mfe=y_mfe,
            y_mae=y_mae,
            ...
            mfe_w_all=mfe_w_all,
            mfe_head_prep=mfe_head_prep,
            mae_head_prep=mae_head_prep,
            range_head_prep=range_head_prep,
            y_r10_fit=y_r10_fit,
            y_r20_fit=y_r20_fit,
            y_ttp_fit=y_ttp_fit,
            r10_head_prep=r10_head_prep,
            r20_head_prep=r20_head_prep,
            ttp_head_prep=ttp_head_prep,
            mh_on=bool(mh_on),
            train_mask=train_mask,
            val_mask=val_mask,
            test_mask=test_mask,
            fit_train_mask=fit_train_mask,
            val_tune_mask=val_tune_mask,
            val_report_mask=val_report_mask,
            straddle_label_stats=straddle_label_stats,
            derived_feature_stats=derived_feature_stats,
            l2_dropped_features=l2_dropped_features,
            tune_frac=tune_frac,
            min_std=min_std,
            skip_hard=skip_hard,
            n_oof=n_oof,
...
def load_l2_trade_decision() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if meta.get("backend") != "pytorch_unified":
        raise RuntimeError(
            "L2: only PyTorch unified checkpoints are supported (meta.backend == 'pytorch_unified'). "
            "LightGBM L2 was removed; retrain layer2."
        )
    import torch

    from core.training.unified.model import UnifiedL2L3Net

    ufile = str(meta.get("unified_model_file", "l2_unified_l2l3.pt"))
    path = os.path.join(MODEL_DIR, ufile)
    net = UnifiedL2L3Net.from_meta(meta)
    state = torch.load(path, map_location="cpu")
    net.load_state_dict(state, strict=True)
    net.eval()
    models: dict[str, Any] = {"unified": net}
    model_files = dict(meta.get("model_files", {}))
    for key, fname in model_files.items():
        if not key.endswith("_calibrator") or not fname:
            continue
        with open(os.path.join(MODEL_DIR, str(fname)), "rb") as f:
...
def infer_l2_trade_decision(
    ...
    n_inf = len(df)
    if models.get("unified") is None or meta.get("backend") != "pytorch_unified":
        raise RuntimeError("L2 inference requires PyTorch unified model (retrain L2; LightGBM L2 removed).")
    from core.training.unified.train import _regression_pred_inverse, infer_l2_unified_raw

    raw = infer_l2_unified_raw(
        models["unified"],
        X,
        meta,
        batch_size=int(os.environ.get("L2_UNIFIED_INFER_BATCH", "16384") or 16384),
    )
    gate_l = raw["gate_logit"]
    p0 = 1.0 / (1.0 + np.exp(-np.clip(gate_l, -20.0, 20.0)))
    trade_p = _apply_binary_calibrator(
        p0.astype(np.float64),
        models.get("trade_gate_calibrator"),
    ).astype(np.float32)
    range_pred = _regression_pred_inverse(raw["range"], aux_prep.get("range"), clip_max=10.0)
    mfe_pred = _regression_pred_inverse(raw["mfe"], aux_prep.get("mfe"), clip_max=5.0)
    mae_pred = _regression_pred_inverse(raw["mae"], aux_prep.get("mae"), clip_max=4.0)
    z0 = np.zeros(n_inf, dtype=np.float32)
    if (meta.get("l2_unified_config") or {}).get("multi_horizon"):
        range10_pred = range20_pred = ttp90_pred = z0
        if "range_10" in raw and raw["range_10"] is not None and aux_prep.get("range_10") is not None:
            range10_pred = _regression_pred_inverse(raw["range_10"], aux_prep["range_10"], clip_max=15.0)
        if "range_20" in raw and raw["range_20"] is not None and aux_prep.get("range_20") is not None:
            range20_pred = _regression_pred_inverse(raw["range_20"], aux_prep["range_20"], clip_max=15.0)
        if "ttp90" in raw and raw["ttp90"] is not None and aux_prep.get("ttp90") is not None:
            ttp90_pred = _regression_pred_inverse(raw["ttp90"], aux_prep["ttp90"], clip_max=1.0)
    else:
        range10_pred = range20_pred = ttp90_pred = z0
    trade_thr = float(meta.get("trade_threshold", 0.5))
    vol_cfg = meta.get("l2_vol_only_config")
    if not isinstance(vol_cfg, dict):
        vol_cfg = {}
    hold_bars = int(meta.get("range_horizon_bars", _l2_range_horizon_bars()))
    rv_lookback_m = int(meta.get("l2_iv_proxy_rv_lookback", vol_cfg.get("iv_proxy_rv_lookback", _l2_iv_proxy_rv_lookback())))
    vixy_z_beta_m = float(meta.get("l2_iv_proxy_vixy_z_beta", vol_cfg.get("iv_proxy_vixy_z_beta", _l2_iv_proxy_vixy_z_beta())))
    vrp_mult_m = float(
        meta.get(
            "l2_rv_to_iv_mult",
            meta.get("l2_vrp_base_mult", vol_cfg.get("rv_to_iv_mult", vol_cfg.get("vrp_base_mult", _l2_rv_to_iv_mult()))),
        )
    )
    implied_range_sigma_mult_m = float(
        meta.get(
            "l2_implied_range_sigma_mult",
            vol_cfg.get("implied_range_sigma_mult", _l2_implied_range_sigma_mult()),
        )
    )
    implied_proxy_range = _l2_implied_proxy_range_atr(
        ...
        frame,
        hold_bars=hold_bars,
        rv_lookback=rv_lookback_m,
        vixy_z_beta=vixy_z_beta_m,
        vrp_base_mult=vrp_mult_m,
        implied_range_sigma_mult=implied_range_sigma_mult_m,
    )
    long_edge_thr = float(meta.get("l2_vol_long_edge_thr", vol_cfg.get("long_edge_thr", _l2_vol_long_edge_thr())))
    short_edge_thr = float(meta.get("l2_vol_short_edge_thr", vol_cfg.get("short_edge_thr", _l2_vol_short_edge_thr())))
    ...
    vol_edge = (
        (np.asarray(range_pred, dtype=np.float64) - np.asarray(implied_proxy_range, dtype=np.float64))
        / np.maximum(np.asarray(implied_proxy_range, dtype=np.float64), 1e-6)
    ).astype(np.float32)
    ...
    vol_regime_score_i = np.zeros(len(df), dtype=np.float32)
    ...
    _n_inf = int(len(vol_signal))
    print(
        f"  [L2 infer] vol_signal (long_vol=+1 / flat=0 / short_vol=-1): "
        f"long={int((vol_signal == 1).sum())} flat={int((vol_signal == 0).sum())} short={int((vol_signal == -1).sum())} / {_n_inf}",
        flush=True,
    )
    if regime_adjust_m:
    ...
    outputs["l2_vol_signal"] = vol_signal.astype(np.int32)
    ...
    outputs["l2_realized_pred_range"] = np.asarray(range_pred, dtype=np.float32)
    outputs["l2_implied_proxy_range"] = np.asarray(implied_proxy_range, dtype=np.float32)
    outputs["l2_vol_edge"] = np.asarray(vol_edge, dtype=np.float32)
    ...
    outputs["l2_vol_regime_score"] = vol_regime_score_i.astype(np.float32)
    ...
    edge_l1b = (
        frame["l1b_edge_pred"].to_numpy(dtype=np.float32, copy=False)
        if "l1b_edge_pred" in frame.columns
        else np.zeros(n_inf, dtype=np.float32)
    )
    outputs["l2_decision_class"] = l2_l3_synthetic_decision_class(
        vol_signal.astype(np.float32),
        edge_l1b.astype(np.float64),
        trade_p.astype(np.float64),
    )
    return outputs
