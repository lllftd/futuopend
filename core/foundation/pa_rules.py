...
from core.foundation.pa_numba_kernels import pullback_counting_fast


# ---------------------------------------------------------------------------
# HTF resampler (closed buckets, causal on 1m)
# ---------------------------------------------------------------------------
...
def _straddle_focused_features(df_1m: pd.DataFrame) -> pd.DataFrame:
    ...
    out["rv_iv_spread_z_60"] = (
        ((rv20 - rv20.rolling(60, min_periods=20).mean()) / (rv20.rolling(60, min_periods=20).std() + 1e-8))
        - ((pk20 - pk20.rolling(60, min_periods=20).mean()) / (pk20.rolling(60, min_periods=20).std() + 1e-8))
    ...
    out["vol_momentum_diff_60"] = (rv20.pct_change(30) - pk20.pct_change(30)).replace([np.inf, -np.inf], np.nan).fillna(0.0).clip(-5.0, 5.0).to_numpy(dtype=np.float64)
    ratio = (rv20 / (pk20 + 1e-8)).replace([np.inf, -np.inf], np.nan).fillna(1.0)
    out["rv_iv_ratio_change_60"] = ratio.pct_change(15).replace([np.inf, -np.inf], np.nan).fillna(0.0).clip(-5.0, 5.0).to_numpy(dtype=np.float64)
...
def _add_pa_features_multi_tf(
    ...
    # Native 1m realized-vol + straddle stack must exist for *all* MTF configs: prep derives
    # ``vol_regime_label`` / L1a gates from ``pa_volume_zscore`` et al. (legacy path always adds these).
...
def add_pa_features(
    ...
    # Same tqdm policy as core.training.common.lgbm_utils._tq (cannot import lgbm_utils: circular).
