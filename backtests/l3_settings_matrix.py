...
from backtests.l3_architecture_gate import architecture_gate_from_metrics


RESULTS_ROOT = REPO_ROOT / "results" / "l3_matrix"
L3_LOG_PATH = REPO_ROOT / "logs" / "layer3.log"
...
MATRIX_CONFIGS: list[dict[str, Any]] = [
    {
        ...
        "env": {
            ...
            "L3_POLICY_FORCE_MODE": "",
        },
    },
    {
        "name": "higher_patience",
        "env": {
    ...
    {
        "name": "force_prob_only",
        "env": {
            "L3_EXIT_EARLY_PATIENCE_STRENGTH_OVERRIDE": "0.10",
            "L3_EXIT_LATE_PRESSURE_STRENGTH_OVERRIDE": "0.18",
            "L3_EXIT_HYST_ENTER_OVERRIDE": "0.55",
            "L3_EXIT_HYST_LEAVE_OVERRIDE": "0.35",
            "L3_POLICY_FORCE_MODE": "prob_only",
        },
    },
    {
        "name": "force_tie_break",
        "env": {
            "L3_EXIT_EARLY_PATIENCE_STRENGTH_OVERRIDE": "0.10",
            "L3_EXIT_LATE_PRESSURE_STRENGTH_OVERRIDE": "0.18",
            "L3_EXIT_HYST_ENTER_OVERRIDE": "0.55",
            "L3_EXIT_HYST_LEAVE_OVERRIDE": "0.35",
            "L3_POLICY_FORCE_MODE": "tie_break",
        },
    },
]
...
def main() -> None:
    ...
    parser.add_argument("--max-runs", type=int, default=6, help="Cap number of matrix configs to run.")
    ...
    for cfg in MATRIX_CONFIGS[:n_runs]:
        ...
        if args.execute:
            if args.retrain_layer3:
                _run(["bash", "scripts/run_train.sh", "layer3"], env=env, cwd=REPO_ROOT)
            _run([sys.executable, "run_in_sample_backtest.py"], env=env, cwd=REPO_ROOT)
        else:
            print("  dry-run commands:")
            if args.retrain_layer3:
                print("   ", "bash scripts/run_train.sh layer3")
