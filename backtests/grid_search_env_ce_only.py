"""
Test CE-only technical gate under PA environments, with different CE params by environment.

Usage:
  python3 -m backtests.grid_search_env_ce_only --start 2025-01-01 --end 2025-12-31 --symbols SPY QQQ
"""
...
OUT_DIR = Path("results") / "env_ce_only_search"
CE_VALID_BARS = 5
...
def _build_params(base: RuleParams) -> RuleParams:
    # Keep PA environment execution, but use CE-only technical gate.
    return replace(
        base,
        pa_use_env_execution=True,
        pa_mtf_trend_filter=False,
        pa_use_advanced_pa_triggers=False,
        pa_env_ce_only_gate=True,
...
def main() -> int:
    ...
    with ProcessPoolExecutor(max_workers=args.max_workers, initializer=_init_worker, initargs=(symbols,)) as pool:
        ...
        pbar = tqdm(total=len(futures), desc="Env CE-only grid", unit="job", dynamic_ncols=True)
    ...
    out_all = out_dir / f"env_ce_only_all_{ts}.csv"
    out_best = out_dir / f"env_ce_only_best_{ts}.csv"
